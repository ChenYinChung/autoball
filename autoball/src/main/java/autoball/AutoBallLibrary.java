 ...
 */
public interface AutoBallLibrary extends StdCallLibrary {
...
//    public static final AutoBallLibrary INSTANCE = (AutoBallLibrary)Native.loadLibrary(AutoBallLibrary.JNA_LIBRARY_NAME, AutoBallLibrary.class, MangledFunctionMapper.DEFAULT_OPTIONS);
    ...
    /**
     * Original signature : <code>BOOL ABDll_Init()</code><br>
     * <i>native declaration : line 74</i>
     */
    boolean ABDll_Init();
    /**
     * AutoBallDll的初始化操作。<br>
     * Original signature : <code>BOOL ConnectReader(int, long)</code><br>
     * <i>native declaration : line 77</i>
     */
    boolean ConnectReader(int nCommNum, NativeLong lBaudrate);
    /**
     ...
     */
    boolean ConnectRD1(int nCommNum, NativeLong lBaudrate);
    /**
     ...
     */
    boolean ConnectRD2(int nCommNum, NativeLong lBaudrate);
    /**
    ...
    /**
     * 斷開與第二臺RD-1000設備的連接，釋放串口<br>
     * Original signature : <code>BOOL StartGame(int, int, int)</code><br>
     * <i>native declaration : line 95</i>
     */
    boolean StartGame(int nGameCount, int nTimeSpan, int nCurGameNum);
    /**
     * 開始開球。參數一 nGameCount表示要連續開球的次數，如果不設置默認為1，即只開一盤；參數nTimeSpan表示下一次開球與上一次開球的時間間隔，默認為0，即開完一盤接著開下一盤;參數nCurGameNum表示指定的開球局的局號，為0時表示不指定局號，接續前一局局號。返回True表示已經啟動開球，false表示出錯未啟動。<br>
     ...
     */
    boolean GetGameInfo(AutoBallLibrary.LPSTR pGameInfo);
    /**
    ...
    /**
     * 繼續掛起的開球進程。<br>
     * Original signature : <code>int AB_GetLastError(LPSTR)</code><br>
     * <i>native declaration : line 113</i>
     */
    int AB_GetLastError(AutoBallLibrary.LPSTR strErrorMessage);
    /**
     ...
     */
    boolean SetControlProcess(AutoBallLibrary.LPSTR strContorlProcess);
    /**
     ...
     */
    boolean SetControlStyle(int nControlStyle);
